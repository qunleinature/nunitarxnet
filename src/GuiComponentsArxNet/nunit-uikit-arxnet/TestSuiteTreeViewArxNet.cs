...
namespace NUnit.UiKit.ArxNet
{
	...
	/// </summary>
	public class TestSuiteTreeViewArxNet : TreeView
	{
        static Logger log = InternalTrace.GetLogger(typeof(TestSuiteTreeViewArxNet));
		...
		public TestSuiteTreeViewArxNet()
		{
			InitializeComponent();
        ...
        {
            string imageSet = ServicesArxNet.UserSettings.GetSetting("Gui.TestTree.AlternateImageSet") as string;
		...
		private void InitializeComponent()
		{
            ...
            System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(TestSuiteTreeViewArxNet));
        ...
        public new bool CheckBoxes
        {
            ...
            { 
                ...
                {
                    VisualStateArxNet visualState = !value && this.TopNode != null
                        ? new VisualStateArxNet(this)
                        : null;
		...
		private void OnTestUnloaded( object sender, TestEventArgs e)
		{
			...
			if ( ServicesArxNet.UserSettings.GetSetting( "Gui.TestTree.SaveVisualState", true ) && loader != null)
				try
				{
					new VisualStateArxNet(this).Save(VisualStateArxNet.GetVisualStateFileName(loader.TestFileName));
				}
				catch(Exception ex)
				{
...
//			}
		...
		private bool IsValidFileDrop( IDataObject data )
		{
			...
			if ( fileNames.Length == 1 )
			{
				...
				if ( ServicesArxNet.UserSettings.GetSetting( "Options.TestLoader.VisualStudioSupport", false ) )
					isProject |= ServicesArxNet.ProjectService.CanConvertFrom( fileName );
		...
		public void Reload( TestNode test )
		{
            ...
            if (result != null && !ServicesArxNet.UserSettings.GetSetting("Options.TestLoader.ClearResultsOnReload", false))
		...
		{
			DisplayStyle initialDisplay = (TestSuiteTreeViewArxNet.DisplayStyle)
				ServicesArxNet.UserSettings.GetSetting( "Gui.TestTree.InitialTreeDisplay", DisplayStyle.Auto );
		...
		private void RestoreVisualState()
		{
            if (loader != null)
            {
                string fileName = VisualStateArxNet.GetVisualStateFileName(loader.TestFileName);
                if (File.Exists(fileName))
                {
                    VisualStateArxNet.LoadFrom(fileName).Restore(this);
                }
            }
        }
    ...
    internal class CheckedTestFinder
	{
		...
		public CheckedTestFinder( TestSuiteTreeViewArxNet treeView )
		{
