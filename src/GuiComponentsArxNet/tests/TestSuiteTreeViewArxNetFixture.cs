...
namespace NUnit.UiKit.ArxNet.Tests
{
	...
	/// </summary>
	/// 
	public class TestSuiteTreeViewArxNetFixture
	{
		protected string testsDll = MockAssembly.AssemblyPath;
		protected Test suite;
        protected TestSuiteTreeViewArxNet treeView;
		...
		public void SetUp() 
		{
			...
			treeView = new TestSuiteTreeViewArxNet();
            treeView.Load(new TestNode(suite));
		}
    }

    [TestFixture]
    public class TestSuiteTreeViewArxNetTests : TestSuiteTreeViewArxNetFixture
    {
		...
		public void BuildFromResult()
		{
			...
			TestSuiteTreeNodeArxNet node = treeView.Nodes[0] as TestSuiteTreeNodeArxNet;
			Assert.AreEqual( testsDll, node.Text );
			Assert.IsNotNull( node.Result, "No Result on top-level Node" );
	
			node = node.Nodes[0].Nodes[0] as TestSuiteTreeNodeArxNet;
			...
			foreach( TestSuiteTreeNodeArxNet child in node.Nodes )
			{
				...
				{
					node = child.Nodes[0] as TestSuiteTreeNodeArxNet;
					...
					TestSuiteTreeNodeArxNet test1 = node.Nodes[2] as TestSuiteTreeNodeArxNet;
					...
					Assert.AreEqual( TestSuiteTreeNodeArxNet.SuccessIndex, test1.ImageIndex );

					TestSuiteTreeNodeArxNet test4 = node.Nodes[5] as TestSuiteTreeNodeArxNet;
					Assert.AreEqual( false, test4.Result.Executed, "MockTest4: Executed" );
					Assert.AreEqual( TestSuiteTreeNodeArxNet.IgnoredIndex, test4.ImageIndex );
					return;
				}
			}
		...
		/// </summary>
		private TestSuiteTreeNodeArxNet FixtureNode( TestSuiteTreeViewArxNet treeView )
		{
			return (TestSuiteTreeNodeArxNet)treeView.Nodes[0].Nodes[0].Nodes[0].Nodes[0].Nodes[0];
		}
		...
		public void SetTestResult()
		{
			...
			TestSuiteTreeNodeArxNet fixtureNode = FixtureNode( treeView );
...
//		}
    ...
    [TestFixture]
    public class TestSuiteTreeViewReloadTests : TestSuiteTreeViewArxNetFixture
    {
        ...
        private void CheckThatTreeMatchesTests(TestSuiteTreeViewArxNet treeView, Test suite, string msg)
        {
            CheckThatNodeMatchesTest((TestSuiteTreeNodeArxNet)treeView.Nodes[0], suite, msg);
        }

        private void CheckThatNodeMatchesTest(TestSuiteTreeNodeArxNet node, Test test, string msg)
        {
            ...
            if (test.IsSuite)
            {
                ...
                //    foreach( TestSuiteTreeNodeArxNet n in node.Nodes )
                ...
                {
                    CheckThatNodeMatchesTest((TestSuiteTreeNodeArxNet)node.Nodes[index], (Test)test.Tests[index], msg);
                }
            }
        }

        private void CheckTreeMap(TestSuiteTreeViewArxNet treeView, Test test, string msg)
        {
            TestSuiteTreeNodeArxNet node = treeView[test.TestName.UniqueName];
